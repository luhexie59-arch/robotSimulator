'''
D* (Dynamic A*) Path Finder - Version 2.3
'''
...
class AStarPlanner:
    ...
    def safetyCheck(self, x, y, radius=10, mobile_range=15):
    ...
    def plan(self):
        ...
        heapq.heappush(pq, (self.h(self.startx, self.starty), (self.startx, self.starty, 0, None)))
        while len(pq):
            _, curr = heapq.heappop(pq)
            if (curr[0] == self.endx and curr[1] == self.endy):
                result = self.waypoints(route)
            ...
            for dx in range(-radius, radius + 1):
                for dy in range(-radius, radius + 1):
                    ...
                    if (0 <= check_x < SCREENSIZE[0] and 0 <= check_y < SCREENSIZE[1]
                        and (constMap[check_x, check_y] == 1 or not self.safetyCheck(check_x, check_y, 5))):
                        return False
        return True
    ...
    def waypoints(self, route):
        ...
        # Iterative path smoothing by removing unnecessary waypoints until no more changes can be made
        smoothed = self.smoothPath(path)
        next_attempt = None
        iterCount = 0
        while True:
            next_attempt = self.smoothPath(smoothed)
            iterCount += 1
            if smoothed == next_attempt:
                break
            else:
                smoothed = next_attempt
        print('iter count:', iterCount, 'result:', next_attempt)
        return next_attempt
...
class mobileObstacle:
    ...
    def move(self):
        ...
        new_x = self.x + dx
        new_y = self.y + dy
        new_x = max(0, min(SCREENSIZE[0] - 20, new_x))
        new_y = max(0, min(SCREENSIZE[1] - 20, new_y))
        self.x = new_x
        self.y = new_y
        self.rect = pygame.Rect(self.x, self.y, 20, 20)
        self.update_varMap()

    def update_varMap(self):
        varMap[:] = np.zeros(SCREENSIZE)
        x0 = max(0, self.x)
        x1 = min(SCREENSIZE[0], self.x + 20)
        y0 = max(0, self.y)
        y1 = min(SCREENSIZE[1], self.y + 20)
        varMap[x0:x1, y0:y1] = 1
...
with open(config_path, 'r') as f:
    maps = json.load(f)[f'map {MAP}']
    obs = maps["obstacles"]
    obstacles = [pygame.Rect(*ob) for ob in obs]
    for ob in obs:
        constMap[ob[0]:ob[0]+ob[2], ob[1]:ob[1]+ob[3]] = 1
...
mobileObs = mobileObstacle(200, 150)
...
while not done:
    ...
    if not arrived:
        ...
        if robot.path:
            ...
            vision_range = 15
            # If not currently waiting for a replan, check ahead in movement direction
            if not waiting_for_replan:
                blocked = False
                if dx != 0 or dy != 0:
