...
class randomObstacle:
    def __init__(self, num:int, interval:float, speedRange):
        ...
        # Set the points along the positive x-axis first
        # y-coordinate follows normal distribution, x-coordinate follows uniform distribution
        y = rng.standard_normal(num) * 20
        x = rng.uniform(0, length, num)
    ...
    def randMove(self):
        self.lastUpdate = time.time()
        randd = rng.choice(self.speedRange, size=(self.num, 2), replace=True)
        self.randList += randd
